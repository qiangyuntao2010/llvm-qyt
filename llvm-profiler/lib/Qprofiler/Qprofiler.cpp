...
#include "corelab/Qprofiler/Qprofiler.h"
...
void Qprofiler::setFunctions(Module &M)
{
	...
	QproInitialize = M.getOrInsertFunction(
			"QproInitialize",
			Type::getVoidTy(Context),
			(Type*)0);

	QproFinalize = M.getOrInsertFunction(
			"QproFinalize",
			Type::getVoidTy(Context),
			(Type*)0);
	
	QproCallSiteBegin = M.getOrInsertFunction(
			"QproCallSiteBegin",
			Type::getVoidTy(Context),
	...
	QproLoopBegin = M.getOrInsertFunction(
			"QproLoopBegin",
			Type::getVoidTy(Context),
			Type::getInt16Ty(Context),
			(Type*)0);
	...
	QproLoopEnd = M.getOrInsertFunction(
			"QproLoopEnd",
			Type::getVoidTy(Context),
			Type::getInt16Ty(Context),
			(Type*)0);
	
	QproDisableCtxtChange = M.getOrInsertFunction(
			"QproDisableCtxtChange",
			Type::getVoidTy(Context),
			(Type*)0);
...
void Qprofiler::setIniFini(Module& M)
{
	...
	CallInst::Create(QproInitialize, actuals, "", entry); 
...
void Qprofiler::addProfilingCodeForLoop(Loop *L, Value *locIDVal){
		...
		for (unsigned int exit_i = 0; exit_i < exitNum; ++exit_i)
		{
			...
			if(find)
			{
				...
				if(exitBB->isLandingPad())
				{
					...
					CallInst::Create(QproLoopEnd, args, "", inst->getNextNode());
				}
				else
				{
					...
					CallInst::Create(QproLoopEnd, args, "", inst);
	...
	args.resize(0);
	CallInst::Create(QproLoopNext, args, "", header->getFirstNonPHI()); 
}
...
void Qprofiler::addProfilingCodeForCallSite(Instruction *invokeOrCallInst, Value *locIDVal){
	...
	if((*found)->isRecursiveCallSiteNode()){
		...
		argsNone.resize(0);
		CallInst::Create(QproDisableCtxtChange, argsNone, "", invokeOrCallInst);
	}
	...
	if(TerminatorInst *termInst = dyn_cast<TerminatorInst>(invokeOrCallInst)){
		...
		if(isa<CallInst>(invokeOrCallInst)){
			...
			if((*found)->isRecursiveCallSiteNode()){
				std::vector<Value*> argsNone(0);
				argsNone.resize(0);
				CallInst::Create(QproEnableCtxtChange, argsNone, "", ci);
			}
		}
			...
			if((*found)->isRecursiveCallSiteNode()){
				std::vector<Value*> argsNone(0);
				argsNone.resize(0);
				CallInst::Create(QproEnableCtxtChange, argsNone, "", ci1);
				CallInst::Create(QproEnableCtxtChange, argsNone, "", ci2);
			}
		}
		else
			assert(0&&"WTF?????");
        ...
        if(CallInst *callInst = dyn_cast<CallInst>(instruction)){
          if(callee->getName() == "malloc"){
            ...
            if(wasBitCasted){// for indirect call bitcast malloc
              Value * changeTo = Builder.CreateBitCast(QproMalloc, ty);
              ...
              CallInst *newCallInst = Builder.CreateCall(QproMalloc, args, "");
              makeMetadata(newCallInst, fullId);
              instruction->replaceAllUsesWith(newCallInst);
              listOfInstsToBeErased.push_back(instruction);
            }
            ...
            args[2] = instrId_;
            if(wasBitCasted){
              Value *changeTo = Builder.CreateBitCast(QproCalloc, ty);
              CallInst *newCallInst = Builder.CreateCall(changeTo, args, "");
              makeMetadata(newCallInst, fullId);
              instruction->replaceAllUsesWith(newCallInst);
              listOfInstsToBeErased.push_back(instruction);
            } else {
              CallInst *newCallInst = Builder.CreateCall(QproCalloc, args, "");
              makeMetadata(newCallInst, fullId);
              instruction->replaceAllUsesWith(newCallInst);
              listOfInstsToBeErased.push_back(instruction);
            }
            ...
            args[2] = instrId_;
            if(wasBitCasted){
              Value *changeTo = Builder.CreateBitCast(QproRealloc, ty);
              CallInst *newCallInst = Builder.CreateCall(changeTo, args, "");
              makeMetadata(newCallInst, fullId);
              instruction->replaceAllUsesWith(newCallInst);
              listOfInstsToBeErased.push_back(instruction);
            } else {
              CallInst *newCallInst = Builder.CreateCall(QproRealloc, args, "");
              makeMetadata(newCallInst, fullId);
              instruction->replaceAllUsesWith(newCallInst);
              listOfInstsToBeErased.push_back(instruction);
            }
...
bool Qprofiler::isUseOfGetElementPtrInst(LoadInst *ld){
  // is only Used by GetElementPtrInst ?
  return std::all_of(ld->user_begin(), ld->user_end(), [](User *user){return isa<GetElementPtrInst>(user);});
}

Value* Qprofiler::castTo(Value* from, Value* to, InstInsertPt &out, const DataLayout *dl)
{
  LLVMContext &Context = module->getContext();
