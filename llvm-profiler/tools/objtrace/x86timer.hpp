...
uint32_t lo, hi;  // lo means low, hi means high
/* We cannot use "=A", since this would use %rax on x86_64 */
__asm__ __volatile__ ("rdtsc" : "=a" (lo), "=d" (hi)); //__asm__ means use assembly language and __volatile__ means the complier do not need to optimize the assembly code, so people always use __asm__ __volatile__ to code the assembly code which do not need to optimize
